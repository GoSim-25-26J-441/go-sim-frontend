...
import { validateEndpointTargets, validateWorkloadTargets } from "@/lib/simulation/scenario-behavior-helpers";
...
export default function ProjectNewSimulationPage() {
  ...
  const validateScenarioStep = (): boolean => {
    ...
    if (draftBlocking) {
      ...
      const wlT = validateWorkloadTargets(scenario);
      if (!wlT.ok) {
        scenarioIssue = wlT.errors[0] ?? "Each workload target must reference an existing endpoint.";
      } else {
        const epT = validateEndpointTargets(scenario);
        if (!epT.ok) {
          scenarioIssue = epT.errors[0] ?? "Each downstream target must reference an existing endpoint.";
        }
      }
    }
  ...
  const executeCreateRun = async (
    ...
    if (scenarioValidationStale) {
      setErrors({
        general:
          "Scenario changed since last validation. Go to step 1, open Debug: scenario YAML & validation, and run Validate Scenario before creating a run.",
      });
      return;
    }
    if (scenarioValidationResult && !scenarioValidationResult.valid) {
      const ok =
        typeof window !== "undefined" &&
        window.confirm(
          "Last backend validation reported issues for this scenario YAML. Create a run anyway? The run may fail or behave unexpectedly."
        );
      if (!ok) return;
    }
          ...
          {currentStep === 1 && (
            <>
              ...
              <ScenarioBehaviorEditor
                ...
                isSampleScenario={isSampleScenario}
                scenarioYaml={scenarioYaml}
                scenarioYamlError={scenarioYamlError}
                scenarioValidationBusy={scenarioValidationBusy}
                scenarioValidationStale={scenarioValidationStale}
                scenarioValidationError={scenarioValidationError}
                scenarioValidationResult={scenarioValidationResult}
                onValidateScenario={() => {
                  void handleValidateScenario();
                }}
              />
              ...
              <div className="flex flex-col gap-3 pt-4 border-t border-border">
                {!scenarioValidationResult && !scenarioValidationBusy && !scenarioValidationError && (
                  <p className="text-xs text-amber-200/90">
                    Scenario has not been validated yet. Validating on step 1 is recommended; create-run is still
                    allowed if you have not run validation.
                  </p>
                )}
                {scenarioValidationStale && (
                  <p className="text-xs text-amber-200/90">
                    Scenario changed since last validation — <span className="font-semibold">create run is blocked</span>{" "}
                    until you re-run Validate Scenario on step 1 (Debug section).
                  </p>
                )}
